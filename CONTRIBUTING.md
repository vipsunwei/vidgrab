...
面向想在本仓库构建、打包、贡献代码的开发者。普通用户请直接看 [README.md](./README.md)。

## 环境要求

- Node ≥ 22.18（脚本用 TS 原生运行，类型剥离）
- pnpm（见 `package.json` 的 `packageManager`）
- Rust 工具链（版本由 `rust-toolchain.toml` 固定）
...
- 构建期来源全部在 GitHub：yt-dlp 固定版本（脚本常量 `YTDLP_VERSION`，须与提交的
  ...
  升级：改常量 → `FORCE_BINARIES=1 pnpm run fetch:binaries` → 提交更新后的 `bin/yt-dlp.exe`。
  CI 对二进制做了 actions/cache，命中后构建不依赖外部下载源。
...
src-tauri/src/cookies.rs       # Cookie 固定存储（Netscape 格式），按站点组管理
...
src-tauri/src/download.rs     # 下载编排：start_download / pause / cancel / delete_task / clear_task_part
src-tauri/src/history.rs      # 历史与文件持久化
